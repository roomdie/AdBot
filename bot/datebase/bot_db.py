...
class DBCommands:
    ...
    async def get_id(self):
        ...
        command = self.GET_ID.format(user_id)
        record = await query(command, False)
        return record

    async def check_referrals(self):
        user_id = types.User.get_current().id
        command = self.CHECK_REFERRALS.format(user_id)
    ...
    async def check_balance(self):
        user_id = types.User.get_current().id
        command = self.CHECK_BALANCE.format(user_id)
        record = await query(command, False)
        return record

    async def add_channel(self, channel, subs):
        user_id = types.User.get_current().id
        command = self.ADD_CHANNEL.format(channel, subs, user_id)
        await query(command, None)
